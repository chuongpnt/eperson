...
class MenuItem
{
     ...
     * @var string
     */
    public static $NAV_TYPE_TREEVIEW = "treeview";
    public static $NAV_TYPE_TREEVIEW_MENU = "treeview-menu";
	
    public static $NAV_TYPE_NAV = "nav";
    public static $NAV_TYPE_NAV_HIDE = "hide";
    ...
    public function __construct(array $menuData = [])
    {
        if(!empty($menuData))
        {
            if($menuData['nav_type'] === self::$NAV_TYPE_NAV || $menuData['nav_type'] === self::$NAV_TYPE_NAV_HIDE)
            {
                ...
                $this->navMenu = $menuData['nav_menu'];
                $this->icon = $menuData['icon'];
                $this->class = isset($menuData['class']) ? $menuData['class'] : '';
                $this->routeType = $menuData['route_type'];
                $this->routeName = $menuData['route_name'];
                $this->isActive = $menuData['route_name'] == Route::currentRouteName();
            }
            else
            {

...
<script>
    export default {
        ...
        mounted() {
            console.log('pages.files.components.FileList.vue');
        ...
        methods: {
            getFullUrl(file, width, action) {
                ...
                return vAPP.APP_URL +`/files/`+file.id+`/preview?w=`+w+`&action=`+act;
            },
            downloadFile(file) {
                window.open(vAPP.APP_URL + '/files/'+file.id+'/download?file_token='+file.file_token);
            },
